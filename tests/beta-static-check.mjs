...
assert.match(source['summary.js'], /<th>工数<\/th><th>履行<\/th>/)
assert.match(source['summary.js'], /escapeHtml\(row\.note \|\| ''\)/)
assert.match(source['summary.js'], /<table class="seiban-detail-table">/)
assert.match(source['style.css'], /\.seiban-detail-table\s*\{[\s\S]*min-width: 900px/)
assert.match(source['style.css'], /\.seiban-detail-table th:nth-child\(2\)[\s\S]*width: 160px/)
assert.match(source['style.css'], /\.seiban-detail-table th:nth-child\(7\)[\s\S]*width: 240px/)
assert.match(source['app.js'], /workTypeName: getSelectedOptionText\('work_type'\)/)
assert.match(source['app.js'], /`作業内容　\$\{workTypeName \|\| '作業内容未設定'\}`/)
