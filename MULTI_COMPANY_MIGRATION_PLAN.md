...
会社管理、`company_admin`、招待制アカウント作成、フロントエンドの会社絞り込み、段階migration、会社単位RLSは `feature/multi-company-onboarding` で実装済みです。本番DBへの適用と2社分離テストは未実施です。

適用時は、この計画に加えて `MULTI_COMPANY_RUNBOOK.md` の具体的な順序と中止条件に従います。2社目の登録禁止は、同Runbookの必須テストがすべて完了するまで継続します。
...
5. NULL件数と不整合を確認する。
6. フロントエンドの全登録処理に会社IDを付け、DBのRLSとトリガーでログイン利用者の会社と一致することを検証する。
...
11. 確認後に `company_id` を `not null` にし、最後に複数社機能フラグを有効化する。
...
フロントエンドが送信した任意の会社IDを信用せず、DBのRLS・会社整合性トリガー・権限確認関数で `auth.uid()` の `user_profiles.company_id` と一致する場合だけ保存します。
