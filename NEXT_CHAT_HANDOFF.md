# 次チャット引き継ぎメモ

このファイルは、新しいチャットで工数管理アプリの続きを進める時に最初に読むための引き継ぎメモです。

## プロジェクト情報

- アプリ名: 工数管理アプリ
- ローカル場所: `/Users/katomikihiko/kosu-app`
- GitHub: https://github.com/mikirealk17-cpu/kosu-app
- Vercel: https://kosu-app-kappa.vercel.app

主な確認URL:

- 工数入力: https://kosu-app-kappa.vercel.app/index.html
- 集計画面: https://kosu-app-kappa.vercel.app/summary.html
- 履歴編集: https://kosu-app-kappa.vercel.app/logs.html
- 管理画面: https://kosu-app-kappa.vercel.app/admin.html

## 現在の大きな判断

まずは、金額・請求機能を前面に出すよりも、作業工数だけを安全に管理できる「工数管理版」として一旦商品化を目指す方針です。

理由:

- 金額、単価、請求、権限管理はミスの影響が大きい
- ログインなしで金額を扱うと、他人の金額が見えるリスクがある
- 工数入力、履歴編集、集計、Excel出力はすでに実用段階に近い
- 商品化の第一段階では「誰が、いつ、どの製番で、何時間作業したか」に絞る方が安全

## 第1段階の商品化方針

工数管理版として重視する機能:

- 工数入力
- 作業者管理
- 製番・設備管理
- 作業内容管理
- 入力履歴編集
- 集計画面
- 明細Excel
- 表示中の集計Excel
- スマホで入力しやすいUI

第1段階では控えめにする機能:

- 単価管理
- 請求確認CSV
- 金額計算
- 請求向けのCSV出力

重要:

- 単価・請求関連のDBやコードは削除しない
- 既存データを壊さないため、不要にテーブルや列を消さない
- 画面上で目立たないようにする、または「準備中」扱いにする方向が安全

## これまで完了した主な内容

- デザイン案1と2を融合し、工数入力、集計、履歴編集のUIを刷新
- iPhone向けに入力欄、ボタン、固定保存ボタンを調整
- 保存完了メッセージを改善
- 作業者管理を追加
- 作業内容管理を追加
- 製番管理を追加
- 元請け管理を追加
- 工数入力ごとに元請けを保存できるようにした
- 大元請けはアプリ画面と単価判定から外した
- 単価マスタ、時間単価、固定単価、請負単価の土台を追加
- 請求確認CSVの土台を追加
- 集計タブを整理し、設備別と作業者日別を削除
- CSV出力を「明細CSV」「表示中の集計CSV」「請求確認CSV」に整理
- ログイン・権限管理の設計書を追加
- ログイン・権限管理用のSQL案を追加
- 工数管理版として一旦商品化する方針に合わせ、管理画面から単価管理を通常導線から外した
- 集計画面のCSV選択から請求確認CSVを外した
- 工数入力画面と入力履歴編集画面から元請け・単価区分を通常入力項目として外した
- 上記変更を `e777cc6 Hide billing features for work tracking release` としてコミットし、本番Vercelへ反映済み
- 未追跡の空ファイル `main` を削除し、作業ツリーを整理した
- 明細CSVと表示中集計CSVについて、2026-06-26〜2026-07-03の実データ8件をSupabase RESTで取得し、金額列なしの列構成で出力対象データが読めることを確認した
- ブラウザ操作ツールでのCSVダウンロード実操作は、本番ページ読み込みがタイムアウトしたため未完了。PCブラウザでの目視ダウンロード確認は次回以降に残す
- 公開キーで `rate_master` と `work_logs` の `unit_price` / `billing_amount` が読めることを確認した。画面からは外れているが、商品化第一段階ではDB権限面の要判断事項
- 商品化前チェックリスト `RELEASE_CHECKLIST.md` を追加した
- 元請け別集計の表示用クエリから金額列を外し、請求確認CSV専用の金額クエリと分離した
- 匿名ユーザーから金額系データを隠すための `SUPABASE_WORK_TRACKING_RELEASE_SECURITY.sql` を追加し、2026-07-09にSupabase SQL Editorで実行した
- SQL実行後、公開キーで `rate_master` と `work_logs` の金額列を取得すると `401 permission denied` になることを確認した
- SQL実行後も通常の工数列は公開キーで `200` になり、取得を継続できることを確認した
- 権限変更後、確認用工数の追加・更新・削除を実行し、すべて成功した。確認用データは削除済み
- 本番の入力画面、履歴編集画面、集計画面がエラーなく読み込めることを確認した
- 集計画面の製番別・製番明細・日別・月別・作業者別・元請け別を切り替え、すべてデータ表示とコンソールエラーなしを確認した
- CSV出力ボタンの操作は行ったが、ブラウザ操作環境でBlobダウンロードイベントを捕捉できなかったため、PCブラウザでの実ファイル目視確認は未完了
- 2026-07-10、工数管理版の商品化第一段階では取引先情報も通常導線に出さない方針にし、管理画面から元請け管理リンク・元請け件数を外した
- 2026-07-10、集計画面から元請け別タブを外した。`billing-companies.html` と元請け別集計用コードは第2段階向けとして残している
- 2026-07-10、ローカル確認で管理画面に元請けリンク・元請け件数が出ないこと、集計画面に元請け別タブ・請求確認CSVが出ないことを確認した
- 2026-07-10、ブラウザのスマホ幅390pxで工数入力、履歴編集、集計、管理画面にページ全体の横はみ出しがないことを確認した。ただしiPhone実機確認は未完了
- 2026-07-10、ブラウザ操作環境ではCSVのBlobダウンロードイベントを捕捉できなかったため、PCブラウザでの実ファイル目視確認は引き続き未完了
- 2026-07-11、本番DBで確認用の作業者・作業内容・製番を追加し、編集、工数入力、工数更新、集計対象取得、工数削除まで確認した。確認用工数と製番は削除済み、確認用作業者と作業内容は非表示化済み
- 2026-07-11、CSVはセル幅を保存できないため、集計画面に列幅付きの明細Excel、表示中集計Excelを追加した。CSV出力は残している
- 2026-07-11、ローカル確認で「明細CSV」「表示中CSV」「明細Excel」「表示中Excel」が表示され、タブ切り替えに合わせて日別CSV・日別Excelなどへ名称が変わることを確認した
- 2026-07-11、ブラウザのスマホ幅390pxで集計画面のファイル出力欄が横はみ出ししないことを確認した
- 2026-07-11、CSV/Excel出力から分の生値である `実働分` 列を削除し、`実働時間` 表示に統一した
- 2026-07-11、iPhone実機で工数入力画面に元請け・単価区分の隠しselectが表示される問題が見つかった。原因はグローバルCSSの `select { display: block; }` が `hidden` 属性を上書きし得るため
- 2026-07-11、`[hidden], .is-hidden { display: none !important; }` を追加し、工数入力・履歴編集の金額関連selectをHTML/JS両方で `hidden`、`disabled`、`aria-hidden`、`is-hidden`、`display:none` にするよう修正した
- 2026-07-11、ローカルのスマホ幅390pxで工数入力画面を確認し、日付の次に製番が表示され、元請け・単価区分が本文にも表示selectにも出ないことを確認した
- 2026-07-11、ローカルのスマホ幅390pxで履歴編集画面を確認し、編集用の元請け・単価区分selectが表示されないことを確認した
- 2026-07-11、iPhone実機でキーボード表示中にスクロールすると固定保存ボタンがちらつく/上下に動く問題が見つかった。原因になり得る `visualViewport.scroll` 追従を外し、リサイズ・フォーカス変化時だけキーボードオフセットを更新するよう修正した
- 2026-07-11、キーボードオフセット更新は `requestAnimationFrame` と8px未満の差分無視を入れ、小さなviewport変動で保存ボタンが動かないようにした。実機での最終確認は次回以降も推奨
- 2026-07-11、上記調整後もiPhone実機では保存ボタンの動く幅が大きくなったため、スマホ時の保存ボタン固定表示とキーボード追従JSを完全に撤去した。保存ボタンはフォーム末尾の通常ボタンとして表示する方針に変更
- 2026-07-13、2026-07-01〜2026-07-13の本番データ2件で、明細CSV/Excel、製番別・製番明細・日別・月別・作業者別の表示中集計CSV/Excelを同じ列生成ロジックで確認した。`実働分`、単価、金額列は含まれず、Excelには列幅用 `colgroup` が入ることを確認した
- 2026-07-13、ブラウザ操作で集計画面のファイル出力ボタンを押し、ボタン表示が `出力しました` になりコンソールエラーなしを確認した。ただしブラウザ操作環境ではBlobダウンロードイベントを捕捉できないため、PCブラウザで保存された実ファイルの目視確認は引き続き未完了
- 2026-07-13、保存ボタン固定解除後、ブラウザのスマホ幅390pxで工数入力、履歴編集、集計を再確認した。3画面ともページ全体の横はみ出しなし。工数入力の保存ボタンは `position: static` でフォーム末尾にあり、固定表示は残っていない。集計のタブ列と表は意図した内部横スクロール領域として残っている
- 2026-07-14、CSV/Excel出力の先頭に「工数集計 明細」「工数集計 製番別」などのタイトル行、期間、絞り込み条件を追加した。作業者・作業内容・製番で絞り込んだ場合は、ファイル名にも `worker-...`、`work-...`、`seiban-...` の識別子が入るようにした
- 2026-07-16、集計画面のファイル出力欄に「出力予定: 工数集計 製番別（CSV）」のようなプレビューを追加した。形式、期間、絞り込み条件も出力前に確認できる
- 2026-07-16、工数保存後の完了メッセージを詳細化し、保存した日付、作業者、製番、設備名、開始終了時間、実働時間を表示するようにした。実データ保存での画面確認は次回以降に残す
- 2026-07-18、保存後の詳細メッセージを画面上部の大きめの確認カードに変更した。成功時は8.5秒表示し、読みやすいように幅、余白、影、文字サイズを強めた
- 2026-07-18、上部表示だと画面移動ボタンを塞ぐため、保存後の詳細メッセージを画面下側表示に変更した
- 2026-07-19、CSV/Excel出力から開始時間、終了時間、休憩1分、休憩2分を外し、実働時間は `2.5` のような小数時間の数値で出力するようにした。CSVは数値を引用符なし、Excelは数値セルを数値形式で出すように調整した
- 2026-07-21、集計画面の通常タブから、商品化第一段階では使わない日別・月別を外した。内部の集計関数は残し、必要になった時に戻しやすい形にしている
- 2026-07-21、集計画面の通常出力からCSV選択肢を外し、明細Excelと表示中集計Excelだけを選べるようにした。CSV生成関数は内部に残し、必要になった時に戻しやすい形にしている
- 2026-07-22、Supabase Auth前提の `login.html`、`login.js`、共通認証ガード `auth.js` を追加した。未ログイン時はログイン画面へ移動し、作業者は工数入力と本人履歴だけ、管理者は集計・Excel出力・各マスタ管理を使える方針にした
- 2026-07-22、作業者権限では工数入力の作業者を本人に固定し、履歴画面は本人分だけ表示、削除ボタンは管理者だけに出すようにした
- 2026-07-22、ログイン確認後に実行するRLS SQLとして `SUPABASE_AUTH_RLS_POLICIES.sql` を追加した。実行すると未ログインの公開キーでは対象テーブルを読めなくなるため、先にAuthユーザーと `user_profiles` の設定確認が必要
- 2026-07-23、Supabase Authユーザー2名を作成し、`user_profiles` に登録済み。加藤さん作業者は `auth_user_id = 4d2abde5-8885-48b2-abef-c4398b714e61`、`worker_id = a2ab523d-2ee3-4db2-94b0-7877002126b0`、`role = worker`。管理者は `auth_user_id = a2db86d7-f1bb-45b3-8cb3-4925d9cfd590`、`role = system_admin`
- 2026-07-23、作業者ログインで、入力画面の作業者が加藤に固定されること、通常リンクが履歴編集のみになること、集計・管理・作業者管理・作業内容管理・製番管理・単価管理・元請け管理へ直接アクセスしても権限エラーになることを確認した
- 2026-07-23、管理者ログインで、集計画面、管理画面、作業者管理、作業内容管理、製番管理が開けることを確認した。Excel出力はユーザーのPCブラウザ上で実ファイル出力確認済み
- 2026-07-23、`SUPABASE_AUTH_RLS_POLICIES.sql` をSupabase SQL Editorで実行済み。実行後、公開キーだけの未ログインRESTアクセスで `work_logs`、`worker_master`、`rate_master` が `401 permission denied` になることを確認した
- 2026-07-23、RLS実行後も管理者ログインで集計画面の8件表示、管理トップの件数表示が動くことを確認した
- 2026-07-24、本番Vercelで未ログイン状態の `index.html`、`summary.html`、`logs.html`、`admin.html` がすべて `login.html?reason=login_required` へ戻ることを確認した
- 2026-07-24、商品化前の確認用データ整理用SQL `SUPABASE_DEMO_DATA_CLEANUP.sql` を追加した。まず確認クエリで対象を見てから、ガード用の `raise exception` ブロックを外して実行する安全設計。まだ本番DBでは未実行
- 2026-07-25、本番Vercelで作業者ログイン状態を再確認した。`billing-companies.html`、`summary.html`、`admin.html`、`rates.html` は直接URLでも「この画面を開く権限がありません」と表示され、通常導線から外れている
- 2026-07-25、本番Vercelの作業者ログインで、工数入力画面は作業者が `加藤` に固定され、表示リンクは `履歴編集` のみ。履歴編集画面は作業者フィルタと編集用作業者が `加藤` 固定で、表示リンクは `工数入力` のみ。単価・請求・金額系の表示は通常導線に出ていない
- 2026-07-25、`SUPABASE_DEMO_DATA_CLEANUP.sql` の削除条件を、マスタ3テーブルの同時 `using` から `exists` ベースに変更した。確認用作業者・作業内容・製番・備考に直接一致するログだけを削除対象にするため、より安全
- 2026-07-25、削除・更新を含まない確認専用SQL `SUPABASE_DEMO_DATA_CONFIRM_ONLY.sql` を追加した。まずはこちらをSupabase SQL Editorで実行し、確認用データだけが表示されるか見る
- 2026-07-25、`SUPABASE_DEMO_DATA_CLEANUP.sql` 実行時に `worker_master.updated_at` が存在しないためエラーになった。整理SQLから `updated_at = now()` を外し、`is_active = false` だけ更新するよう修正した。エラー時点ではトランザクションが失敗しているため、削除処理は完了していない前提
- 2026-07-25、`SUPABASE_DEMO_DATA_CLEANUP.sql` 実行時に確認用製番が `rate_master` から参照されていて外部キーエラーになった。第1段階では製番削除を行わず、確認用工数ログ削除と作業者・作業内容の非表示だけに変更した。確認用製番が残るのは正常扱い
- 2026-07-30、Supabase SQL Editorで `SUPABASE_DEMO_DATA_CLEANUP_RUN.sql` を実行し、確認用工数ログの実行後確認 `remaining_demo_logs = 0` を確認した。確認用作業者・作業内容は非表示化し、確認用製番は参照整合性を優先して残している
- 2026-07-30、製番も削除ではなく非表示で運用できるように `SUPABASE_SEIBAN_ACTIVE_SETUP.sql` を追加した。`seiban_master.is_active` を追加し、`CSV-DEMO-*` / `CSV確認_*` / `Codex確認*` 系の確認用製番を非表示にするSQL。製番管理画面は「削除」ではなく「非表示/再表示」に変更済み
- 2026-07-30、工数入力・履歴編集・集計の製番選択は、`seiban_master.is_active = true` の製番だけ表示するように変更した。SQL未実行環境でも壊れないよう、`is_active` 列がない場合は従来クエリへフォールバックする
- 2026-07-31、本番Vercelの管理者ログインで `SUPABASE_SEIBAN_ACTIVE_SETUP.sql` 反映後の状態を確認した。製番管理では `CSV-DEMO-001` / `CSV-DEMO-002` / `CSV-DEMO-003` が `非表示` になり、ボタンは `再表示` になっている
- 2026-07-31、本番Vercelの集計画面を再読み込みし、通常の製番フィルタから `CSV-DEMO` / `CSV確認` / `Codex確認` 系が消え、`全製番`、`A A`、`B B`、`OSZ-1000 テスト反転機` だけが表示されることを確認した
- 2026-07-31、本番Vercelの管理者ログインで履歴編集画面も確認した。管理者用リンク、全作業者フィルタ、編集・削除ボタンが表示され、製番フィルタから確認用CSV-DEMO系が消えている
- 2026-07-31、本番Vercelを390px幅のブラウザで確認した。工数入力、履歴編集、集計、管理画面はいずれもページ全体の横はみ出しなし。通常画面にCSV、単価、請求、金額系の文言は出ていない
- 2026-07-31、ログイン失敗時の案内を改善し、ログイン画面からパスワード再設定メールを送れるようにした。再設定メールのリンク先として `update-password.html` / `update-password.js` を追加し、新しいパスワード登録後はログイン画面へ戻す
- 2026-07-31、Supabase Authentication の Redirect URLs に `https://kosu-app-kappa.vercel.app/update-password.html` を追加済み。Vercel本番で `update-password.html` が開けることも確認済み
- 2026-08-03、実メールのパスワード再設定リンクから新パスワードを登録し、管理者で再ログインできることを確認した
- 2026-08-03、本番用の作業者・作業内容・製番マスタを確認し、確認用項目は削除せず非表示のまま整理済みと判断した
- 2026-08-03、保存・更新の連打防止、編集時の時間帯重複警告、重複確認通信失敗時の安全停止、0件Excel案内、ログインredirect許可リストを作業用コピーへ実装した
- 2026-08-03、監査ログSQL、バックアップ・復旧手順、複数会社移行計画、β版公開状況を追加した。本番DBへの監査SQL適用とVercelデプロイは未実施
- 2026-08-04、上記のβ安全修正と追加ドキュメントを元リポジトリ `/Users/katomikihiko/kosu-app` へ反映し、ローカルコミット `Improve beta release safety` を作成した。GitHub pushは認証情報未設定で失敗したため、Vercelデプロイと本番DBへの監査SQL適用は未実施
- 2026-08-04、作業者による生産番号登録対応を元リポジトリへ反映し、ローカルコミット `Add worker production number registration` を作成した。`production-number-utils.mjs`、工数入力/履歴編集の候補検索・仮登録、製番管理の未確認一覧・確認済み更新・統合導線、`SUPABASE_SEIBAN_PRODUCTION_NUMBER_SETUP.sql`、静的テストを追加した。push、Vercelデプロイ、本番DB適用は未実施
- 2026-08-05、`deffb69 Fix seiban visibility error handling` までGitHub push・Vercel反映済み。DB変更前に正規化後の空キー・重複だけを確認する読み取り専用SQL `SUPABASE_SEIBAN_PRODUCTION_NUMBER_CONFIRM_ONLY.sql` を追加した
- 2026-07-31、通常ログイン中に `update-password.html` を直接開いても更新フォームが有効にならないよう修正した。Supabaseの `PASSWORD_RECOVERY` イベントが来た場合だけ有効化する
- 2026-08-01、本番Vercelの `https://kosu-app-kappa.vercel.app/update-password.html` を直接開き、パスワード入力欄と更新ボタンが無効、メッセージが「再設定リンクが確認できませんでした。ログイン画面から再設定メールを送り直してください。」になることを確認した

## 現在残っている重要な注意点

### 金額・請求まわり

単価管理と請求確認CSVは実装途中の機能として扱う。
商品化第一段階では、通常利用者に強く見せない方が安全。

理由:

- 金額は間違えると信用問題になる
- 元請けや作業者ごとの閲覧制御がまだない
- ログインなし運用では、URLを知っている人が管理画面に入れる可能性がある

### ログイン・権限管理

フロント側のログイン画面と権限ガードは追加済み。
Supabase側でAuthユーザーと `user_profiles` を設定し、RLS SQLも実行済み。

設定済みユーザー:

- 作業者 加藤: `auth_user_id = 4d2abde5-8885-48b2-abef-c4398b714e61`、`worker_id = a2ab523d-2ee3-4db2-94b0-7877002126b0`、`role = worker`
- 管理者: `auth_user_id = a2db86d7-f1bb-45b3-8cb3-4925d9cfd590`、`role = system_admin`

関連ファイル:

- `AUTH_PERMISSION_DESIGN.md`
- `SUPABASE_AUTH_PERMISSION_SETUP.sql`
- `SUPABASE_AUTH_RLS_POLICIES.sql`
- `SUPABASE_DEMO_DATA_CONFIRM_ONLY.sql`
- `SUPABASE_DEMO_DATA_CLEANUP.sql`
- `SUPABASE_DEMO_DATA_CLEANUP_RUN.sql`
- `SUPABASE_SEIBAN_ACTIVE_SETUP.sql`

実行済み:

1. `SUPABASE_AUTH_PERMISSION_SETUP.sql`
2. Supabase Authユーザー作成
3. `user_profiles` 設定
4. 管理者・作業者ログイン確認
5. `SUPABASE_AUTH_RLS_POLICIES.sql`

次に必要な確認:

- 管理者で、集計画面と履歴編集画面は再確認済み。Excel出力はユーザーのPCブラウザで実ファイル出力確認済み
- 確認用データ整理は `SUPABASE_DEMO_DATA_CLEANUP_RUN.sql` で実行済み。`remaining_demo_logs = 0` を確認済み
- 確認用製番は `rate_master` などの参照があるため削除していない。`SUPABASE_SEIBAN_ACTIVE_SETUP.sql` 実行後、通常導線からは外れていることを本番で確認済み
- パスワード再設定は、通常ログイン中や直URLでは更新フォームが有効にならないこと、実メールの再設定リンクから新パスワードを登録して再ログインできることを確認済み

### 大元請け

一度追加したが、現在の運用には合わないため画面から外した。
既存DBの旧テーブル・旧列は、データ破損を避けるため削除していない。
現行画面と単価判定では使わない。

## 次にやると良いこと

商品化第一段階として、次は以下の順番が安全。

1. GitHub認証を解決し、ローカルコミットを `origin/main` へpushしてVercelへデプロイする
2. Supabaseの現在プランとバックアップ保持状況を確認し、β開始前バックアップを取得する
3. `SUPABASE_SEIBAN_PRODUCTION_NUMBER_CONFIRM_ONLY.sql` を実行し、空キー0件・重複0件を確認する
4. `SUPABASE_SEIBAN_PRODUCTION_NUMBER_SETUP.sql` を本番へ適用する
5. `SUPABASE_BETA_AUDIT_SETUP.sql` をテスト環境で確認後、本番へ適用する
6. iPhone実機で工数入力、履歴編集、集計、ファイル出力、生産番号候補検索・仮登録の操作感を確認する。390px幅のブラウザ確認では横はみ出しなし
7. 管理者/作業者の本番権限、保存連打、編集時重複警告、生産番号の確認済み更新・統合を再確認する

おすすめ方針:

- 削除ではなく非表示から始める
- DB構造は残す
- 請求機能は第2段階として残す
- 工数管理版の安定性を先に高める

## 直近コミット

- `b2dce55 Add auth role separation`
- `db376b6 Add seiban visibility management`
- `0aa733d Use Excel-only summary exports`
- `2098f51 Hide daily monthly summary tabs`
- `78a041a Restrict password update to recovery flow`
- `d47bee7 Add release checklist`
- `e777cc6 Hide billing features for work tracking release`

注意:

- `b2dce55` はGitHubへpush済みで、本番Vercelへの反映対象。
- `SUPABASE_AUTH_RLS_POLICIES.sql` は2026-07-23にSupabase SQL Editorで実行済み。
- 新しいチャットでは `git status --short` と `git log -1 --oneline` を確認してから作業する。

## 未追跡ファイル

以前あった `main` という未追跡の空ファイルは削除済み。
新しいチャットでは `git status --short` が空に近い状態か確認する。

## 作業ルール

- 工数アプリのファイルだけ変更する
- 勝手に仕様変更しない
- データ破損につながる変更をしない
- DBのテーブルや列は安易に削除しない
- Supabase権限変更は、画面動作への影響を確認してから実行する
- 金額に関わる機能は慎重に扱う
- 変更したら必要に応じて README / PROJECT / TODO / CHANGELOG を更新する
- 完了後は、変更ファイル、変更内容、テスト結果、初心者向け説明を報告する

## 次チャットで最初に依頼すると良い文

```text
/Users/katomikihiko/kosu-app/NEXT_CHAT_HANDOFF.md を読んで、工数管理版として一旦商品化する方針で続きを進めてください。
Supabase Auth、user_profiles、RLS、確認用データ整理、本番用マスタ確認、実メールのパスワード再設定、β安全修正、生産番号登録対応の元リポジトリ反映とローカルコミットは完了済みです。GitHub pushは認証情報未設定で未完了です。次は `SUPABASE_SEIBAN_PRODUCTION_NUMBER_SETUP.sql` の重複確認と適用、push・Vercelデプロイ、iPhone実機確認から進めてください。
```
