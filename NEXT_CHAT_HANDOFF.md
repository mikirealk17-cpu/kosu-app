...
- パスワード再設定は、通常ログイン中や直URLでは更新フォームが有効にならないことを本番で確認済み。実メールの再設定リンクから新パスワードを登録できるかは未確認
...
1. 実メールでパスワード再設定リンクを受け取り、`update-password.html` へ戻って新パスワードを設定できることを確認する
2. 本番用の作業者・製番・作業内容マスタを、実運用に合わせて整える
3. iPhone実機で工数入力、履歴編集、集計、ファイル出力の操作感を確認する。390px幅のブラウザ確認では横はみ出しなし
4. 商品化前の最終確認として、管理者/作業者の本番権限確認をもう一度軽く通す
...
Supabase Authの管理者・作業者ユーザー、user_profiles設定、RLS SQL実行、確認用データ整理、製番非表示、パスワード再設定ページの直URL保護は完了済みです。まずは実メールでのパスワード再設定リンク確認、本番用マスタ整理、iPhone実機確認から進めてください。
```
