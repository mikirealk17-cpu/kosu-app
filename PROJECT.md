...
工数管理版として公開する前に `SUPABASE_WORK_TRACKING_RELEASE_SECURITY.sql` を実行し、匿名ユーザーから単価マスタと金額列を読めない・書けない状態にします。
...
- `billing-companies.html`: 元請け管理画面（第2段階向け、通常導線からは外す）
...
工数管理版の公開前セキュリティSQLは `SUPABASE_WORK_TRACKING_RELEASE_SECURITY.sql` に分けています。

SQL実行後の動き:

- `workers.html` で作業者を追加する
- 第2段階では `billing-companies.html` で元請けを追加する
...
- 第2段階では `summary.html` の元請け別タブで元請けごとの工数を確認する
...
- `billing_company_master`: 第2段階向け。工数管理版では通常導線から外す
